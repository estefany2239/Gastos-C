...
import { 
  Save, AccountBalanceWallet, Logout, DeleteOutline, Edit, Close 
...
const Dashboard = ({ setIsAuthenticated }) => {
  ...
  const [currentId, setCurrentId] = useState(null);  // ID del registro a editar
      ...
      <Container maxWidth="lg">
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
          ...
          <Button 
            ...
            sx={{ background: GOLD_GRADIENT, borderRadius: '15px', px: 3, textTransform: 'none', fontWeight: 800 }}
          >
            Cerrar Sesión
          </Button>
        </Stack>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={8}>
            <Paper elevation={0} sx={{ p: 4, borderRadius: '25px', border: '1px solid rgba(212, 175, 55, 0.2)', mb: 3 }}>
              <Typography variant="h6" sx={{ mb: 3, color: GOLD_DARK, display: 'flex', alignItems: 'center', gap: 1, fontWeight: 700 }}>
                <AccountBalanceWallet sx={{ color: GOLD_PRIMARY }} /> 
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                  {isEditing && (
                    <Button variant="outlined" onClick={cancelarEdicion} startIcon={<Close />} sx={{ borderRadius: '12px', px: 4, color: 'grey.600', borderColor: 'grey.400' }}>
                      CANCELAR
                    </Button>
                  )}
              ...
              <Table size="medium">
                <TableHead sx={{ bgcolor: GOLD_LIGHT }}>
                  <TableRow>
                    ...
                    <TableCell sx={{ fontWeight: 800, color: GOLD_DARK }} align="center">Acciones</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {listaDeGastos.length === 0 ? (
                    <TableRow><TableCell colSpan={5} align="center" sx={{ py: 6, color: 'text.disabled' }}>No hay registros disponibles</TableCell></TableRow>
                  ) : (
                    listaDeGastos.map((item) => (
                      <TableRow key={item.id} hover>
                        ...
                        <TableCell align="center">
                          <IconButton onClick={() => handlePreparaEdicion(item)} size="small" sx={{ color: GOLD_DARK }}>
                            <Edit fontSize="small" />
                          </IconButton>
                          <IconButton onClick={() => handleEliminar(item.id)} size="small" sx={{ color: '#d32f2f' }}>
          ...
          <Grid item xs={12} md={4}>
            <Card elevation={0} sx={{ p: 4, borderRadius: '25px', border: `1.5px solid ${GOLD_PRIMARY}`, bgcolor: '#fff' }}>
